...
import { getAllMarketplaceProperties } from '@/lib/demo-marketplace';
...
/** Hand-crafted showcase + marketplace listings (10 agents + 10 companies × 8 each per country). */
function buildFullDemoPropertyCatalog(): Property[] {
  const byId = new Map<string, Property>();
  for (const row of HANDCRAFTED_DEMO_PROPERTIES) {
    byId.set(row.id, row);
  }
  for (const row of getAllMarketplaceProperties()) {
    byId.set(row.id, row);
  }
...
export function listDemoProperties(q: DemoPropertyQuery): {
  ...
  if (q.sort === 'price-asc' || q.sort === 'price_asc') rows.sort((a, b) => a.price - b.price);
  else if (q.sort === 'price-desc' || q.sort === 'price_desc') rows.sort((a, b) => b.price - a.price);
